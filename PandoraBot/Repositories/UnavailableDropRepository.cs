...
public sealed class UnavailableDropRepository : IDropRepository
{
    ...
    public Task<IReadOnlyList<EnemyDropRow>> GetEnemyDropsAsync(bool includeDeleted = false) => throw CreateException();
    ...
    public Task<EnemyDropRow> UpdateDropAsync(string enemyId, string itemName, EnemyDropCreateInput input) => throw CreateException();
    public Task<EnemyDropRow> DeleteDropAsync(string enemyId, string itemName) => throw CreateException();
    public Task<EnemyDropSettingRow> UpsertDropSettingAsync(EnemyDropSettingInput input, bool allowDuplicate) => throw CreateException();
}
