...
관리자 명령어는 Discord 서버 관리 권한을 가진 진행자용입니다.

- `/관리목록`: 저장소에 등록된 캐릭터 목록 요약 조회
- `/관리정보`: Discord User ID와 캐릭터 이름으로 특정 캐릭터 조회
...
현재 캐릭터 관리, 상태창 이미지, 플레이어 도움말, 기본 판정 명령, 관리자용 기본 운영 명령까지 구현되어 있습니다. 다음 단계는 판정 로그 저장, 전투 상태 관리, 몬스터 데이터 관리입니다.
