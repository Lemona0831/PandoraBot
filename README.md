...
캐릭터 등록, 선택, 조회, 상태창 이미지 출력, 기본 판정, 관리자용 캐릭터 관리를 통해 어반 판타지 커뮤니티 운영 흐름을 자동화하는 것을 목표로 합니다.
...
현재 캐릭터 관리, 상태창 이미지, 기본 판정 명령까지 구현되어 있습니다. 다음 단계는 판정 로그 저장, 전투 상태 관리, HP 변경, 몬스터 데이터 관리입니다.
